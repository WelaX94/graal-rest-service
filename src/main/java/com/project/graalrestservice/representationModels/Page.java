...
public class Page<T extends List<?>> extends RepresentationModel<Page<T>> {
    ...
    public void setLinks(int pageSize, String status, String nameContains, boolean orderByName, boolean reverseOrder) {
        if (page > 1) add(linkTo(methodOn(ScriptsController.class).getScriptListPage(page - 1, pageSize, status, nameContains, orderByName, reverseOrder)).withRel("previousPage").expand());
        if (page < numPages) add(linkTo(methodOn(ScriptsController.class).getScriptListPage(page + 1, pageSize, status, nameContains, orderByName, reverseOrder)).withRel("nextPage").expand());
    }
